            ...
            this.Invoke((MethodInvoker)delegate
            {
                ...
                while(true)
                {
                    ...
                    else if (e.Node.Level >= 3)
                    {
                        ...
                        txtTermLabels.Text = GenUtil.LabelsToString(term.Labels, term.Name);
            ...
            this.Invoke((MethodInvoker)delegate
            {
                ...
                while(true)
                {
                    ...
                    if (curNode.Level >= 3)
                    {
                        ...
                        {
                            // term labels
                            string ignoreTermName = txtTermName.Text.Trim();
                            List<string> lstNewLabels = txtTermLabels.Text.Trim().Split(new char[] { ';' }).ToList();

                            // delete existing labels (if label in collection not found in textbox)
                            int i = 0;
                            while (i < term.Labels.Select(x => x.Value).ToList().Count())
                            {
                                if (term.Labels[i].Value.Trim().ToLower() != ignoreTermName.Trim().ToLower()
                                    && !lstNewLabels.Any(x => x.Trim().ToLower() == term.Labels[i].Value.Trim().ToLower())
                                    && !term.Labels[i].IsDefaultForLanguage)
                                {
                                    term.Labels[i].Delete();
                                }
                                else
                                {
                                    i++;
                            ...
                            {
                                if (!string.IsNullOrEmpty(lblNew.Trim())
                                    && lblNew.Trim().ToLower() != ignoreTermName.Trim().ToLower()
                                    && !term.Labels.Any(x => x.Value.Trim().ToLower() == lblNew.Trim().ToLower()))
                                {
                            ...
                            term.Name = txtTermName.Text.Trim();
                            term.TermStore.CommitAll();

                            curNode.Text = string.Format("{1} [{0}]", term.TermsCount, txtTermName.Text.Trim());
            ...
            this.Invoke((MethodInvoker)delegate
            {
                while(true)
                {
                    ...
                    if (curNode.Level == 2)
                    {
                        ...
                        try
                        {
                            tSet.Name = txtTermSetName.Text.Trim();
                            ...
                            curNode.Text = string.Format("{1} [{0}]", tSet.Terms.Count, txtTermSetName.Text.Trim());
        ...
        private void btnCreateTermSet_Click(object sender, EventArgs e)
        {
            ...
            this.Invoke((MethodInvoker)delegate
            {
                while(true)
                {
                    ...
                    if (tGroup != null)
                    {
                        if (tGroup.TermSets.Any(x => x.Name.Trim().ToLower() == txtTermSetName.Text.Trim().ToLower()))
                        {
                        ...
                        {
                            // create new termset
                            TermSet tSet = tGroup.CreateTermSet(txtTermSetName.Text.Trim());
                            tSet.IsAvailableForTagging = chkTermSetIsAvailableForTagging.Checked;
                            tSet.Description = txtTermSetDescription.Text.Trim();
                            tSet.TermStore.CommitAll();
                            ...
                            // add termset to tree
                            TreeNode newNode = new TreeNode();
                            newNode.Text = string.Format("{0} [0]", tSet.Name);
                ...
                while(true)
                {
                    ...
                    try
                    {
                        if (curNode.Level == 2)
                        {
                            ...
                            {
                                newTerm = tSet.CreateTerm(txtTermName.Text.Trim(), CultureInfo.CurrentCulture.LCID);
                            }
                            else
                            {
                                ...
                                {
                                    newTerm = tSet.CreateTerm(txtTermName.Text.Trim(), CultureInfo.CurrentCulture.LCID, new Guid(txtNewTermGuid.Text));
                                }
                            }
                        ...
                        else if (curNode.Level >= 3)
                        {
                            ...
                            if (GenUtil.IsNull(txtNewTermGuid.Text)
                                ...
                                || !GenUtil.IsGuid(txtNewTermGuid.Text))
                            {
                                newTerm = term.CreateTerm(txtTermName.Text.Trim(), CultureInfo.CurrentCulture.LCID);
                            }
                            else
                            {
                                if (!GenUtil.IsGuid(txtNewTermGuid.Text))
                                ...
                                {
                                    newTerm = term.CreateTerm(txtTermName.Text.Trim(), CultureInfo.CurrentCulture.LCID, new Guid(txtNewTermGuid.Text));
                                }
                            }
                        ...
                        if (newTerm != null)
                        {
                            ...
                            {
                                foreach (string lbl in txtTermLabels.Text.Trim().Split(new char[] { ';' }))
                                {
                                    if (!GenUtil.IsNull(lbl)
                                        && lbl.Trim().ToLower() != newTerm.Name.Trim().ToLower())
                                    {
                                        newTerm.CreateLabel(lbl.Trim(), CultureInfo.CurrentCulture.LCID, false);
                                    }
                                }
                            }
                            ...
                            // add term to tree
                            TreeNode newNode = new TreeNode();
                            newNode.Text = string.Format("{0} [0]", newTerm.Name);
